...
class ArticleServiceImpl(
    ...
    override fun getArticles(): List<Article> {
        return articleRepository.findAll().map { Article(it.id!!, it.title, it.content) }
    }
    ...
    override fun saveArticle(article: ArticleRequest) {
        articleRepository.save(ArticleEntity(title = article.title, content = article.content))
    }
}
