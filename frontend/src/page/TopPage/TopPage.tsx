          ...
          {articles.map((article) => {
            return (
              <div className="my-6 justify-start" key={article.id}>
                <h2>
                  <Link className="text-2xl font-medium leading-4" to={`/articles/${article.id}`}>
                    {article.title}
                  </Link>
                </h2>
                <p className="my-2">{article.content.length <= 200 ? article.content : `${article.content.substring(0, 200)}...`}</p>
              </div>
            )
          })}
        </div>
